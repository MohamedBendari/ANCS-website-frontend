<template>
  <section class="team">
    <div class="container">
      <div class="section-header">
        <span class="subtitle">Our Team</span>
        <h1>Meet The Developers</h1>
        <p>The talented people behind ANCS who made it all possible</p>
      </div>
      <div class="team-grid">
        <!-- Mohamed Bendary -->
        <div class="team-card">
  <div class="card-image">
    <img src="../assets/mohamed.jpg" alt="Mohamed Bendary" class="avatar-img">
    <div class="social-links">
      <a href="https://github.com/MohamedBendari"><i class="fab fa-github"></i></a>
      <a href="https://www.linkedin.com/in/mohamed-bendary-6778922ba?utm_source=share&utm_campaign
      =share_via&utm_content=profile&utm_medium=android_app"><i class="fab fa-linkedin"></i></a>
      <a href="mailto:mohamedsharkawy078@gmail.com">
  <i class="fas fa-envelope"></i>
</a>
    </div>
  </div>
  <div class="card-info">
    <h3>Mohamed Bendary</h3>
    <span class="role fullstack">Full Stack Developer</span>
    <p>
      Building modern web applications using Django & Python for backend
      and creating responsive, user-friendly interfaces for frontend.
    </p>
    <!-- 🔵 Frontend -->
    <div class="skills-section">
      <div class="skills frontend">
        <span><i class="fab fa-html5"></i> HTML</span>
        <span><i class="fab fa-css3-alt"></i> CSS</span>
        <span><i class="fab fa-js"></i> JavaScript</span>
        <span><i class="fab fa-vuejs"></i> Vue</span>
      </div>
    </div>
    <!-- 🟢 Backend -->
    <div class="skills-section">
      <div class="skills backend">
        <span><i class="fas fa-server"></i> Django</span>
        <span><i class="fab fa-python"></i> Python</span>
        <span><i class="fas fa-database"></i> Database</span>
      </div>
    </div>
  </div>
</div>
       <!-- Mohamed Ehab -->
<div class="team-card">
  <div class="card-image">

    <img src="../assets/mohamed ehab.jpg" alt="Mohamed Ehab" class="avatar-img">

    <div class="social-links">
      <a href="https://github.com/MOHAmED3467" target="_blank">
        <i class="fab fa-github"></i>
      </a>

      <a href="https://www.linkedin.com/in/mohamed-ehab-sayed-mahmoud-ict-587322345?utm_source=share_via&utm_content=profile&utm_medium=member_android" target="_blank">
        <i class="fab fa-linkedin"></i>
      </a>

      <a href="mailto:ma644064@gmail.com">
        <i class="fas fa-envelope"></i>
      </a>
    </div>

  </div>

  <div class="card-info">
    <h3>Mohamed Ehab</h3>

    <span class="role">
      Frontend & Mobile Application Developer
    </span>

    <p>
      Developed responsive user interfaces and enhanced the user experience
      using Vue.js, Flutter, and modern frontend technologies.
    </p>

    <div class="skills">
      <span>Vue.js</span>
      <span>Flutter</span>
      <span>CSS3</span>
      <span>Firebase</span>
      <span>UI/UX</span>
    </div>
  </div>
</div>
<!-- Zyad Ahmed -->
<div class="team-card">
  <div class="card-image">

    <img src="../assets/zyad.jpg" alt="Zyad Ahmed" class="avatar-img">

    <div class="social-links">

      <a href="https://github.com/zyada7med2" target="_blank">
        <i class="fab fa-github"></i>
      </a>

      <a href="https://www.linkedin.com/in/zyad-ahmed/" target="_blank">
        <i class="fab fa-linkedin"></i>
      </a>

      <a href="mailto:zyada7med@gmail.com">
        <i class="fas fa-envelope"></i>
      </a>

    </div>
  </div>

  <div class="card-info">

    <h3>Zyad Ahmed</h3>

    <span class="role fullstack">
      NetDevOps Engineer
    </span>

    <p>
      Led the network automation architecture and AI integration for the application.
      Designed the Cisco IOS configuration engine, automated complex routing workflows
      (OSPF, EIGRP), and engineered the AI Copilot's network intelligence to act as
      an automated network assistant.
    </p>

    <div class="skills backend">

      <span><i class="fas fa-network-wired"></i> Cisco IOS</span>

      <span><i class="fas fa-project-diagram"></i> OSPF</span>

      <span><i class="fas fa-route"></i> EIGRP</span>

      <span><i class="fas fa-robot"></i> AI Copilot</span>

      <span><i class="fas fa-server"></i> NetDevOps</span>

      <span><i class="fas fa-terminal"></i> Automation</span>

    </div>

  </div>
</div>

        <!-- Youssef Samir -->
<div class="team-card">
  <div class="card-image">
        <img src="../assets/youssefsamir.jpg" alt="youssefsamir" class="avatar-img">
    <div class="social-links">
      <a href="https://github.com/youssefsamir14"><i class="fab fa-github"></i></a>
      <a href="https://www.linkedin.com/in/youssef-samir-b12320292/"><i class="fab fa-linkedin"></i></a>
      <a href="mailto:yousefsamir2004@gmail.com">
        <i class="fas fa-envelope"></i>
      </a>
    </div>
  </div>
  <div class="card-info">
    <h3>Youssef Samir</h3>
    <span class="role backend">Python Developer</span>
    <p>
      Python developer specialized in building desktop applications,
      working with GUI frameworks, system communication, and network automation tools.
    </p>
    <!-- 🟢 Skills -->
    <div class="skills-section">
      <div class="skills backend">
        <span><i class="fab fa-python"></i> Python 3.7+</span>
        <span><i class="fas fa-desktop"></i> PySide (GUI)</span>
        <span><i class="fas fa-network-wired"></i> Paramiko (SSH)</span>
        <span><i class="fas fa-plug"></i> PySerial (Serial)</span>
        <span><i class="fas fa-code"></i> Requests (API)</span>
      </div>
    </div>
  </div>
</div>
<!-- Youssef Marai Ali Salem -->
<div class="team-card">

  <div class="card-image">

    <img src="../assets/youssef-marai.jpg" alt="Youssef Marai Ali Salem" class="avatar-img">

    <div class="social-links">

      <a href="#" target="_blank">
        <i class="fab fa-github"></i>
      </a>

      <a href="#" target="_blank">
        <i class="fab fa-linkedin"></i>
      </a>

      <a href="mailto:youssef@example.com">
        <i class="fas fa-envelope"></i>
      </a>

    </div>

  </div>

  <div class="card-info">

    <h3>Youssef Marai Ali Salem</h3>

    <span class="role backend">
      Backend & Network Automation Developer
    </span>

    <p>
      Contributed to backend development and network automation systems,
      focusing on secure communication, device management, and automation workflows
      for enterprise network environments.
    </p>

    <div class="skills backend">

      <span><i class="fab fa-python"></i> Python</span>

      <span><i class="fas fa-server"></i> Backend</span>

      <span><i class="fas fa-network-wired"></i> Networking</span>

      <span><i class="fas fa-project-diagram"></i> Automation</span>

      <span><i class="fas fa-database"></i> SQLite</span>

      <span><i class="fas fa-shield-alt"></i> Security</span>

    </div>

  </div>

</div>
<!-- Ahmed Abdelkarim -->
<div class="team-card">
  <div class="card-image">
    <img src="../assets/Ahmed Mahmoud.jpg" alt="Ahmed Abdelkarim" class="avatar-img">

    <div class="social-links">

      <a href="https://github.com/ahmaadmahmouud394-hub" target="_blank">
        <i class="fab fa-github"></i>
      </a>

      <a href="https://www.linkedin.com/in/ahmed-abdelkarim-5ab1681b9?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app" target="_blank">
        <i class="fab fa-linkedin"></i>
      </a>

      <a href="mailto:ahmaadmahmouud394@gmail.com">
        <i class="fas fa-envelope"></i>
      </a>

    </div>
  </div>

  <div class="card-info">
    <h3>Ahmed Abdelkarim</h3>

    <span class="role fullstack">Network Automation & QA Engineer</span>

    <p>
      Developed and implemented automated testing and network validation systems
      for ANCS, focusing on secure protocol automation, configuration parsing,
      and GNS3-based network simulation environments.
    </p>

    <!-- 🟢 Skills -->
    <div class="skills-section">
      <div class="skills backend">
        <span><i class="fab fa-python"></i> Python 3.x</span>
        <span><i class="fas fa-vial"></i> unittest</span>
        <span><i class="fas fa-network-wired"></i> Paramiko</span>
        <span><i class="fas fa-terminal"></i> Telnetlib3</span>
        <span><i class="fas fa-plug"></i> PySerial</span>
        <span><i class="fas fa-desktop"></i> PySide6</span>
        <span><i class="fas fa-project-diagram"></i> GNS3 API</span>
        <span><i class="fas fa-robot"></i> Gemini AI API</span>
        <span><i class="fas fa-database"></i> SQLite</span>
      </div>
    </div>

  </div>
</div>
<!-- Aliaa Sabry Mahmoud Mohamed -->
<div class="team-card">
  <div class="card-image">
    <img src="../assets/Aliaa Sabry.jpg" alt="Aliaa Sabry" class="avatar-img">
    <div class="social-links">
      <a href="https://github.com/aliaammm050-cell"><i class="fab fa-github"></i></a>
      <a href="https://www.linkedin.com/in/aliaa-sabry-08272a404"><i class="fab fa-linkedin"></i></a>
      <a href="mailto:aliaammm050@gmail.com">
        <i class="fas fa-envelope"></i>
      </a>
    </div>
  </div>
  <div class="card-info">
    <h3>Aliaa Sabry</h3>
    <span class="role frontend">Frontend Developer</span>
    <p>
      Frontend developer passionate about creating clean, responsive,
      and user-friendly web interfaces using modern web technologies.
    </p>
    <!-- 🔵 Frontend Skills -->
    <div class="skills-section">
      <div class="skills frontend">
        <span><i class="fab fa-html5"></i> HTML</span>
        <span><i class="fab fa-css3-alt"></i> CSS</span>
        <span><i class="fab fa-js"></i> JavaScript</span>
        <span><i class="fas fa-mobile-alt"></i> Responsive Design</span>
      </div>
    </div>
  </div>
</div>
      </div>
    </div>
  </section>
</template>

<style scoped>
/* Role */
.role.fullstack {
  display: inline-block;
  padding: 6px 14px;
  border-radius: 20px;
  background: linear-gradient(135deg, #30bee6, #17447e);
  color: var(--text-primary);
  font-size: 12px;
  margin-bottom: 10px;
}
/* Sections */
.skills-section {
  margin-top: 15px;
}
.skills-section h4 {
  font-size: 13px;
  color: #aaa;
  margin-bottom: 8px;
}
/* Skills base */
.skills {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}
/* Frontend */
.skills.frontend span {
  background: rgba(0, 150, 255, 0.1);
  border: 1px solid rgba(0, 150, 255, 0.3);
  color: #00aaff;
}
/* Backend */
.skills.backend span {
  background: rgba(0, 255, 150, 0.1);
  border: 1px solid rgba(0, 255, 150, 0.3);
  color: #00d084;
}
/* Skill item */
.skills span {
  padding: 6px 10px;
  border-radius: 15px;
  font-size: 12px;
  display: flex;
  align-items: center;
  gap: 5px;
}
/* Hover effect 🔥 */
.team-card:hover {
  transform: translateY(-8px);
  transition: 0.3s;
  box-shadow: 0 20px 40px rgba(0,0,0,0.4);
}
.team {
  min-height: 100vh;
  padding: 120px 20px 80px;
  background: var(--page-bg-gradient);
}
.container {
  max-width: 1200px;
  margin: 0 auto;
}
.section-header {
  text-align: center;
  margin-bottom: 80px;
}
.subtitle {
  display: inline-block;
  padding: 8px 20px;
  background: rgba(66, 165, 245, 0.1);
  border: 1px solid rgba(66, 165, 245, 0.3);
  border-radius: 50px;
  color: #42a5f5;
  font-size: 14px;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 1px;
  margin-bottom: 20px;
}
.section-header h1 {
  color: var(--text-primary);
}
.section-header p {
  font-size: 20px;
  color: var(--text-muted);
  max-width: 600px;
  margin: 0 auto;
}
.team-grid {
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 30px;
}
.team-card {
  background: var(--card-bg);
  border: 1px solid var(--border-color);
  border-radius: 24px;
  overflow: hidden;
  transition: all 0.4s ease;
}
.team-card:hover {
  transform: translateY(-10px);
  background: var(--card-bg-hover);
  border-color: rgba(66, 165, 245, 0.2);
  box-shadow: 0 20px 40px rgba(0, 0, 0, 0.3);
}
.card-image {
  position: relative;
  height: 220px;
  background: linear-gradient(135deg, rgba(66, 165, 245, 0.1), rgba(0, 119, 182, 0.1));
  display: flex;
  align-items: center;
  justify-content: center;
  overflow: hidden;
}
.avatar {
  width: 100px;
  height: 100px;
  background: linear-gradient(135deg, #42a5f5, #0077b6);
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 36px;
  font-weight: 700;
  color: var(--text-primary);
  border: 4px solid rgba(255, 255, 255, 0.1);
  transition: all 0.4s ease;
}
.avatar-img {
  width: 100%;
  height: 100%;
  object-fit: cover;
    object-position: top center;
    display: block;
    border-radius: 0;
}
.card-image {
    position: relative;
    height: 280px;
    background: linear-gradient(135deg, rgba(66, 165, 245, 0.1), rgba(0, 119, 182, 0.1));
    display: flex;
    align-items: center;
    justify-content: center;
    overflow: hidden;
}

.social-links {
  position: absolute;
  bottom: -50px;
  left: 50%;
  transform: translateX(-50%);
  display: flex;
  gap: 15px;
  transition: all 0.4s ease;
}
.team-card:hover .social-links {
  bottom: 20px;
}
.social-links a {
  width: 40px;
  height: 40px;
  background: rgba(255, 255, 255, 0.1);
  backdrop-filter: blur(10px);
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--text-primary);
  text-decoration: none;
  transition: all 0.3s ease;
  border: 1px solid rgba(255, 255, 255, 0.2);
}
.social-links a:hover {
  background: #42a5f5;
  transform: translateY(-3px);
}
.card-info {
  padding: 30px;
  text-align: center;
}
.card-info h3 {
  font-size: 22px;
  font-weight: 700;
  margin-bottom: 8px;
  color: var(--text-primary);
}
.role {
  display: inline-block;
  padding: 6px 16px;
  background: rgba(66, 165, 245, 0.1);
  border: 1px solid rgba(66, 165, 245, 0.3);
  border-radius: 50px;
  color: #42a5f5;
  font-size: 13px;
  font-weight: 600;
  margin-bottom: 15px;
}
.card-info > p {
  font-size: 14px;
  color: var(--text-secondary);
  line-height: 1.6;
  margin-bottom: 20px;
}
.skills {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  justify-content: center;
}
.skills span {
  padding: 5px 12px;
  background: var(--card-bg);
  border: 1px solid var(--border-color);
  border-radius: 20px;
  font-size: 12px;
  color: var(--text-muted);
}
/* Responsive */
@media (max-width: 1200px) {
  .team-grid {
    grid-template-columns: repeat(2, 1fr);
  }
}
@media (max-width: 768px) {
  .section-header h1 {
    font-size: 36px;
  }
  .team-grid {
    grid-template-columns: 1fr;
    max-width: 400px;
    margin: 0 auto;
  }
}
@media (max-width: 600px) {
  .team {
    padding: 90px 20px 60px;
  }
  .section-header {
    margin-bottom: 40px;
  }
  .section-header h1 {
    font-size: 28px;
  }
  .section-header p {
    font-size: 16px;
  }
  .team-grid {
    max-width: 100%;
  }
  .team-card {
    padding: 20px;
  }
  .team-card h3 {
    font-size: 16px;
  }
  .team-card p {
    font-size: 13px;
  }
  .member-role {
    font-size: 12px;
  }
}
</style>
